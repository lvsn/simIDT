...
def get_test_data(rgb_path, depth_path, cam_path, cad_path, seg_path, det_score_thresh, cfg):
    ...
    whole_depth = whole_depth[1:-1]
    whole_pts = get_point_cloud_from_depth(whole_depth, K)

    mesh : trimesh.Scene = trimesh.load_mesh(cad_path)
    print(mesh.scale)
    model_points = mesh.sample(cfg.n_sample_model_point).astype(np.float32)
    if mesh.scale > 1:
        print('apply scale')
...
if __name__ == "__main__":
    ...
    input_data, img, whole_pts, model_points, detections = get_test_data(
        cfg.rgb_path, cfg.depth_path, cfg.cam_path, cad_path_real, cfg.seg_path, 
    ...
    print(pred_rot[valid_masks])
    print(pred_trans[valid_masks])
    print(pred_rot)
    print(pred_trans)
    print(K)
    print(img.shape)
