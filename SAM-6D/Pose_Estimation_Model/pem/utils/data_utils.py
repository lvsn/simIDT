...
try:
    ...
    from angles import euler2mat
except:
    from pem.utils.transform import Transform
    from pem.utils.angles import euler2mat

def compute_error(pose, gt_pose):
    '''
    @pose: 4x4 ndarray
    @gt_pose: 4x4 ndarray
    '''
    gt_trans = Transform.from_matrix(gt_pose)
    est_trans = Transform.from_matrix(pose)
    diff = get_pose_difference(est_trans, gt_trans)
    pose_diff = diff[np.newaxis, :]
    return compute_pose_diff(pose_diff)
    
def get_pose_difference(prediction, ground_truth, signed=False):
    ...
    difference[3:] = rotation.to_parameters(isDegree=True)[3:]
    difference[:3] = prediction_params[:3] - ground_truth_params[:3]
    if signed:
        return difference*-1
