...
from pem.utils.model_utils import (
    LayerNorm2d,
...
class ViT_AE(nn.Module):
    def __init__(self, cfg,) -> None:
        ...
        if self.pretrained:
            ...
            # Quick fix dégueulasse
            if not os.path.exists(vit_checkpoint):
                vit_checkpoint = os.path.join(os.getcwd(), 'Pose_Estimation_Model/pem/checkpoints', 'mae_pretrain_'+ self.vit_type +'.pth')
            # WARNING : I think this is a bug. vit_checkpoint is actually a file
            elif not os.path.isdir(vit_checkpoint):
