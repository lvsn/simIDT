...
def run_inference(segmentor_model, output_dir, cad_path, rgb_path, depth_path, cam_path, stability_score_thresh):
    ...
    template_dir = os.path.join(cad_path, 'templates')
    ...
    save_image(templates, f"{output_dir}/templates.png", nrow=7)
    save_image(templates, f"{template_dir}/masks.png", nrow=7)
    ...
    print(model.ref_data['descriptors'].shape)
    print(model.ref_data['appe_descriptors'].shape)
    ...
    # detections_vis = detections["masks"].cpu().numpy().astype(int)
    # visualize_masks(rgb, detections_vis, save_path=f"{output_dir}/seg.png")
    ...
    # Filter to get only top x detections since i OOM now with a stability threshold of 0.5
    # update detections
    ...
    template_poses[:, :3, 3] *= 0.4 # pq 0.4???
    poses = torch.tensor(template_poses).to(torch.float32).to(device)
    model.ref_data["poses"] =  poses[load_index_level_in_level2(0, "all"), :, :]

    cad_file = [f for f in os.listdir(cad_path) if f.endswith('.ply') or f.endswith('.obj')][0]
    cad_path_real = os.path.join(cad_path, cad_file)
    print(f"cad_path_real: {cad_path_real}")
    mesh = trimesh.load_mesh(cad_path_real)
    print(mesh.scale)
    if mesh.scale < 1:
    ...
    detections.filter(final_score > 0.5)
    detections.to_numpy()
