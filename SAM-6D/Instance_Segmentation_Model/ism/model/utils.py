...
import logging
from ism.utils.inout import save_json, load_json, save_npz
from ism.utils.bbox_utils import xyxy_to_xywh, xywh_to_xyxy, force_binary_mask
import time
...
def convert_npz_to_json(idx, list_npz_paths):
    ...
    print(detections["category_id"])
    print(detections["score"])
    print(detections["time"])
    print(detections["segmentation"])
    # detections["score"] = detections["score"].tolist()
    # detections["category_id"] = detections["category_id"].tolist()
    # detections["time"] = detections["time"].tolist()
