...
from ism.utils.poses.pose_utils import get_obj_poses_from_template_level
...
class Instance_Segmentation_Model(pl.LightningModule):
    ...
    def best_template_pose_multi_object(self, scores):
        _, best_template_idxes = torch.max(scores, dim=-1)
        # N_query, N_object = best_template_idxes.shape[0], best_template_idxes.shape[1]
        # pred_idx_objects = pred_idx_objects[:, None].repeat(1, N_object)
        ...
        return best_template_idxes

    def compute_semantic_score_multi_object(self, proposal_decriptors):
        # print('in compute_semantic_score')
        ...
        # print('aggre:', self.matching_config.aggregation_function)
        ...
        # print('score_per_proposal_and_object:', score_per_proposal_and_object.shape)
        # print('score_per_proposal_and_object:', score_per_proposal_and_object)

        best_templates_idxes = self.best_template_pose_multi_object(scores)

        return score_per_proposal_and_object, best_templates_idxes
    ...
    def compute_semantic_score(self, proposal_decriptors):
        ...
        # print('score_per_proposal:', score_per_proposal.shape)
        # print('score_per_proposal:', score_per_proposal)
        ...
        threshold = self.matching_config.confidence_thresh
        num_top_scores = 50 # try to do a parameter with this

        # Select indices over the threshold
        idx_selected_proposals = torch.arange(len(score_per_proposal), device=score_per_proposal.device)[score_per_proposal > threshold]

        # Limit the selection to the top 50 scores if there are more than 50
        if len(idx_selected_proposals) > num_top_scores:
            _, top_indices = torch.topk(score_per_proposal[idx_selected_proposals], k=num_top_scores)
            idx_selected_proposals = idx_selected_proposals[top_indices]
    ...
    # def compute_appearance_score_multi_view(self, best_templates_idxes, query_appe_descriptors):
    #     # pred_objects_idx = torch.arange(best_templates_idxes.shape[1], device=best_templates_idxes.device)
    #     # con_idx = torch.concatenate((pred_objects_idx[None, :], best_templates_idxes), dim=0)
    ...
    #     print(ref_appe_descriptors.shape)
    #     print(query_appe_descriptors.shape)
    #     query_appe_descriptors = query_appe_descriptors.unsqueeze(1).repeat(1, ref_appe_descriptors.shape[0], 1, 1)
    #     print(query_appe_descriptors.shape)
    #     appe_scores = aux_metric.compute_similarity(query_appe_descriptors, ref_appe_descriptors)
    ...
    def compute_appearance_score_multi_view(self, best_templates_idxes, query_appe_descriptors):
        ...
        print('pred_objects_idx:', pred_objects_idx.shape)
        print('best_templates_idxes:', best_templates_idxes)
        print('best_templates_idxes:', best_templates_idxes.shape)
        con_idx = torch.concatenate((pred_objects_idx[None, :], best_templates_idxes), dim=0)
        ref_appe_descriptors = self.ref_data["appe_descriptors"][con_idx[0, ...], con_idx[1, ...], ...] # N_query x N_patch x N_feature

        aux_metric = MaskedPatch_MatrixSimilarity(metric="cosine", chunk_size=64)
        # query_appe_descriptors = query_appe_descriptors.unsqueeze(1).repeat(1, ref_appe_descriptors.shape[0], 1, 1)
        ref_appe_descriptors = ref_appe_descriptors.unsqueeze(1)
        appe_scores = aux_metric.compute_similarity(query_appe_descriptors, ref_appe_descriptors)
        return appe_scores, ref_appe_descriptors

    def compute_appearance_score_multi_object(self, best_templates_idxes, query_appe_descriptors):
        """
    ...
    def compute_appearance_score(self, best_pose, pred_objects_idx, qurey_appe_descriptors):
        ...
        Based on the best template, calculate appearance similarity indicated by appearance score
        """
        print('in compute_appearance_score')
        print(pred_objects_idx)
        print(pred_objects_idx[None, :])
        print(best_pose)
        print(best_pose[None, :])
