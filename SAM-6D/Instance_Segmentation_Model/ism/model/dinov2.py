...
import logging
import numpy as np
from ism.utils.bbox_utils import CropResizePad, CustomResizeLongestSide
from torchvision.utils import make_grid, save_image
from ism.model.utils import BatchedData
...
class CustomDINOv2(pl.LightningModule):
    ...
    def process_rgb_proposals_batch(self, images_batch, masks_batch, boxes_batch):
        rgbs = torch.from_numpy(images_batch).to(masks_batch.device).float() / 255.0
        rgbs = rgbs.permute(0, 3, 1, 2)
        mean = torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1).to(rgbs.device)
        std = torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1).to(rgbs.device)
        rgbs = (rgbs - mean) / std
        
        masked_rgbs = rgbs * masks_batch.unsqueeze(1)
        processed_masked_rgbs = self.rgb_proposal_processor(
            masked_rgbs, boxes_batch
        )  # [N, 3, target_size, target_size]
        return processed_masked_rgbs

    @torch.no_grad()
    def forward_batch(self, image_batch, proposals):
        processed_rgbs = self.process_rgb_proposals_batch(
            image_batch, proposals.masks, proposals.boxes
        )
    ...
    def forward(self, image_np, proposals):
        ...
        # print(f"proposals.masks: {proposals.masks.shape}")
        # print(f"proposals.boxes: {proposals.boxes.shape}")
        processed_rgbs = self.process_rgb_proposals(
            image_np, proposals.masks, proposals.boxes
        )
        # print(f"processed_rgbs: {processed_rgbs.shape}")
        processed_masks = self.process_masks_proposals(proposals.masks, proposals.boxes)
        if len(processed_masks.shape) == 2:
            processed_masks = processed_masks.unsqueeze(0)
        # print(f"processed_masks: {processed_masks.shape}")
        ...
        cls_features = BatchedData(batch_size=self.chunk_size)
        patch_features = BatchedData(batch_size=self.chunk_size)
        # print(f"batch_rgbs: {len(batch_rgbs)}")
        # print(f"batch_masks: {len(batch_masks)}")
        # print(f"batch_rgbs[0]: {batch_rgbs[0].shape}")
        # print(f"batch_masks[0]: {batch_masks[0].shape}")
